...
export function passwordResetEmailTemplate(data: {
  ...
  return `
    <html lang="en">
      <body style="margin:0;padding:24px;background:#f8fafc;font-family:Arial,sans-serif;color:#0f172a;">
        <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;padding:32px;">
          <h1 style="margin:0 0 16px;color:#0B1F3A;font-size:24px;">Reset your PayAssure password</h1>
          <p>Hello ${escapeHtml(data.username)},</p>
          <p>Use the six-digit password reset OTP below to choose a new password:</p>
          <p style="padding:16px;background:#f1f5f9;border-radius:8px;font-size:24px;font-weight:700;letter-spacing:4px;text-align:center;">${escapeHtml(data.otp)}</p>
          <p style="color:#475569;font-size:14px;">This OTP expires on ${escapeHtml(data.expiresAt)} and can be used only once. If you did not request this, you can ignore this email.</p>
          <p>Thank you,<br />PayAssure</p>
        </div>
      </body>
    </html>
